...
static int is_operator(int character)
{
...
{
	return isspace(character) || is_operator(character) || !character;
}
...
struct token tokenize(struct context* ctx)
{
	...
	if (isdigit(*ctx->text)) {
		...
		if (conversion_end != ctx->text) {
			fatal_error(
				&token, "failed to convert integer constant \33[1;31m%.*s\33[0m",
				(int)(ctx->text - contents), contents);
		}
		return token;
	}
	if (is_operator(*ctx->text)) {
		token.type = get_operator_type(*ctx->text);
		next(ctx);
		return token;
	}
	if (strchr("Rr", *ctx->text)) {
		token.type = TOKEN_TYPE_REGISTER;

		next(ctx);
		if (strchr("Zz", *ctx->text)) {
			next(ctx);
		...
		char* conversion_end = NULL;
		const long long value = (int64_t)strtoll(contents, &conversion_end, 10);
		if (conversion_end != ctx->text || value < 0 || value > MAX_USER_REGISTER) {
			fatal_error(
				&token, "invalid register index \33[1;31m%.*s\33[0m",
				(int)(ctx->text - contents), contents);
		}
	...
	do {
		next(ctx);
	} while (!is_separator(*ctx->text) && *ctx->text != '.');
...
uint64_t get_integer(const struct token* token, int64_t min, int64_t max)
{
	...
	if (value < min || value > max) {
		fatal_error(token, "integer %d is out of range [%d, %d]", value, min, max);
	}
