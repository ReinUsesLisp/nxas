...
#define INSTR(name, func) void parse_##func(struct context *ctx, struct instruction *instr);
#include "instructions/list.inc"
#undef INSTR

static void assemble_predicate(const struct token *token, struct instruction *instr, size_t shift,
                               int is_negable)
{
